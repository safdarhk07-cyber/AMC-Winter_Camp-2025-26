...
# P = 500, I = 500, D = 500
# i tried different values too but i seem to be getting the minimum time to get to the target height with only the extreme values. 
# i know its theoretically wrong but its just what i am getting. 
...
plane = p.loadURDF("PID/plane.urdf")     #Loading the plane
dronePos = [0,0,0.2]                     #Initial Position of the drone
drone = p.loadURDF("PID/quadrotor.urdf", dronePos)      #Loading the drone
...
while(True):
    ...
    if keycode.get(p.B3G_RETURN) == 1:        #If ENTER key is pressed then the simulation executes
        ...
        plane = p.loadURDF("PID/plane.urdf")        #Loading the plane and drone again
        dronePos = [0,0,0.1]
        drone = p.loadURDF("PID/quadrotor.urdf", dronePos)
        ...
        while(True):
            ...
            if keycode.get(p.B3G_RETURN) == 1:    #Reseting the simulation when Enter is pressed
                ...
                plane = p.loadURDF("PID/plane.urdf")
                dronePos = [0,0,0.2]
                drone = p.loadURDF("PID/quadrotor.urdf", dronePos)
                p.stepSimulation()
                break